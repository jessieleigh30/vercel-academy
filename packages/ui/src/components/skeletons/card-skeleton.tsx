 ...
 */
export function CardSkeleton({ className, showImage = true, imageAspect = 'video', textLines = 3 }: CardSkeletonProps) {
  const aspectClasses = {
  ...
  return (
    <div className={cn('bg-white rounded-lg border border-gray-100 overflow-hidden', className)}>
      {showImage && <BaseSkeleton className={cn('w-full', aspectClasses[imageAspect])} />}
